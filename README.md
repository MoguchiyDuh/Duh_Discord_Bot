...
- **Music**: YouTube and SoundCloud playback (prefix a search with `sc:` for SoundCloud) with queue, playlists, lyrics, loop (track/queue), play-next, and a persistent player card with a dynamic button layout: the now-playing page shows transport (prev/pause/skip, seek ±15s), the queue and history pages show ◀ ▶ pagination (10 tracks per page), and a 🎶 📜 🕘 nav row with the active page highlighted switches between them. Shared controls (loop, shuffle, add song, volume, lyrics, stop, 📻 mix toggle) stay in fixed rows. YT mixes/radio pull dynamically: a batch plays, your added tracks jump ahead, the next batch loads only when the queue drains. The card edits in place, shows a log of recent actions, and becomes a session-stats summary when playback ends. Volume is displayed 0-150% with 100% mapped to 20% raw gain (default 100%). Stream URLs are resolved at play time, so long queues never expire.
...
- **Polls**: `/poll question choices minutes multi` creates a poll with button voting (comma-separated choices, 2-10), a live countdown in the footer, per-option vote bars, single-choice vote switching (press again to unvote) or multi-choice toggling, and a final results embed with the winner highlighted. Active polls live in memory and end early if the bot restarts.
